...
export class DriverVehicleService {
  ...
  GetDriverByVehicle(vehicle: string, page: number, searchText: string, numberDis: number) {
    return this.httpService.getRequest(`DriverVehicle/getDriverByVehicle/${vehicle}?page=${page}&Keyword=${searchText}&pageSize=${numberDis}`)
      .pipe(map((data: lstDriverVehicle) => {
        return data;
      }))
  }
